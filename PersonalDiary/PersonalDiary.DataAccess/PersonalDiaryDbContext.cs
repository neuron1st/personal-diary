...
public class PersonalDiaryDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<DiaryEntryEntity>().HasOne(x => x.User)
        ...
        modelBuilder.Entity<FolderOfEntryEntity>().HasOne(x => x.Folder)
            .WithMany(x => x.FoldersOfEntry)
            .HasForeignKey(x => x.FolderId);
        modelBuilder.Entity<FolderOfEntryEntity>().HasOne(x => x.DiaryEntry)
            .WithMany(x => x.Folders)
            .HasForeignKey(x => x.DiaryEntryId);

        // Tag of entry
        modelBuilder.Entity<TagOfEntryEntity>().HasKey(x => new { x.TagId, x.DiaryEntryId });
        modelBuilder.Entity<TagOfEntryEntity>().HasOne(x => x.Tag)
            .WithMany(x => x.Tags)
            .HasForeignKey(x => x.TagId);
        modelBuilder.Entity<TagOfEntryEntity>().HasOne(x => x.DiaryEntry)
            .WithMany(x => x.Tags)
            .HasForeignKey(x => x.DiaryEntryId);
    }
}